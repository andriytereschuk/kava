	...
	function getEventsData(data, limit, year, month, day, direction){
		...
		flags.wrap.find('.eventsCalendar-list').animate({
					...
					if (limit === 0 || limit > i) {
						...
						if ((month === false || month == eventMonth)
								...
								if (month === false && eventDate < new Date()) {
									...
									if (event.url) {
										var eventTitle = '<a href="'+event.url+'" target="' + eventLinkTarget + '" class="articleTitle">' + event.title + '</a>';
									} else {
										var eventTitle = '<span class="articleTitle">'+event.title+'</span>';
									}
			...
			if (!events.length) {
				...
				flags.wrap.find('.eventsCalendar-subtitle').css('opacity',0);
			}
			else {
				flags.wrap.find('.eventsCalendar-subtitle').css('opacity',1);
			}
