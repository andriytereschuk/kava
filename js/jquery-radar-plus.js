  ...
  var Radar = (function() {
    ...
    Radar.prototype.draw = function() {
      ...
      for(var key in this.settings.values)
        size += 1;
      for(var i = 0; i <= this.settings.fixedMaxValue; i+=this.settings.step) {
        this.cxt.beginPath();
		...
		for(j=1; j<=10; j++) { 
			var x = this.width / 2 + Math.cos((Math.PI * 2) * (j / size)) * spacing * i;
			var y = this.height / 2 + Math.sin((Math.PI * 2) * (j / size)) * spacing * i;
			if (first) {
			  this.cxt.moveTo(x, y);
			  end.x = x;
			  end.y = y;
      ...
      grad.addColorStop(1,"rgba("+this.settings.color[0]+","+this.settings.color[1]+","+this.settings.color[2]+",0)");
      this.cxt.fillStyle = grad;
      this.cxt.shadowBlur = 2;
      this.cxt.strokeStyle = "rgba(213,65,65,65)";
      this.cxt.lineWidth = 4;
      this.cxt.shadowColor = "rgba(0, 0, 0, 0)";
      this.cxt.stroke();
      this.cxt.fill();
      
      //this.newCanvas('labels',1000);
      var additionalLineDistance =  false;
      i = 0;
      $.each(this.settings.values, function(key,val){
		//svitkavy
		val = val/2;
        //that.newCanvas('label-'+i, i * 250);
        that.cxt.fillStyle = "rgba(119,119,119,1)";
        that.cxt.strokeStyle = "rgba(0,0,0,0)";
        that.cxt.font = "bold 14px";
        var dist = Math.min(spacing * val, size * spacing);
        var x = that.width / 2 + Math.cos((Math.PI * 2) * (i / size)) * (spacing / 1.2) * val;
        var y = that.height / 2 + Math.sin((Math.PI * 2) * (i / size)) * (spacing / 1.2) * val;

        var textX = that.width / 2 + Math.cos((Math.PI * 2) * (i / size)) * (spacing / 1.1) * max;
        var textY = that.height / 2 + Math.sin((Math.PI * 2) * (i / size)) * (spacing / 1.4) * max * 1.5;
